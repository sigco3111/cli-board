...
import CLIApp from './components/cli/CLIApp'; // CLI 모드 컴포넌트 추가
...
// UI 모드를 저장하기 위한 로컬 스토리지 키 (CLI 또는 PC)
const UI_MODE_KEY = 'mac_board_ui_mode';
...
const App: React.FC = () => {
  ...
  // UI 모드 상태 관리 (CLI 또는 PC)
  const [uiMode, setUiMode] = useState<UIMode>(() => {
    const savedMode = localStorage.getItem(UI_MODE_KEY);
    return (savedMode === 'cli' || savedMode === 'pc') ? savedMode as UIMode : 'cli'; // 기본값은 CLI 모드
  });
   ...
   */
  const toggleUIMode = () => {
    setUiMode(prev => prev === 'cli' ? 'pc' : 'cli');
  };
  ...
  /**
   * 선택된 UI 모드에 따른 컴포넌트 렌더링
   */
  const renderUIByMode = () => {
    if (!isUserLoggedIn || !effectiveUser) {
      // 로그아웃 상태 - 로그인 화면 표시
      return <LoginScreen onLogin={handleLogin} />;
    }

    // 로그인 상태 - 선택된 UI 모드에 따라 다른 컴포넌트 표시
    if (uiMode === 'cli') {
      return (
        <CLIApp 
          user={effectiveUser} 
  ...
  return (
    <>
      <Routes>
        ...
        {/* 메인 앱 라우트 */}
        <Route path="/" element={renderUIByMode()} />
